...
<div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl">
    <h1 class="text-xl font-bold text-gray-800 mb-6">Edit Payment: {{ $payment->invoice_number }}</h1>
        ...
        <div class="space-y-4">
            <div>
                ...
                <select name="member_id" required
                    ...
                    @foreach($members as $member)
                        <option value="{{ $member->id }}" {{ old('member_id', $payment->member_id) == $member->id ? 'selected' : '' }}>{{ $member->name }} ({{ $member->member_id }})</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Invoice Number *</label>
                <input type="text" name="invoice_number" value="{{ old('invoice_number', $payment->invoice_number) }}" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Transaction Date *</label>
                <input type="date" name="transaction_date" value="{{ old('transaction_date', $payment->transaction_date->format('Y-m-d')) }}" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
            <div>
                ...
                <select name="membership_period" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
                    <option value="1 Month" {{ old('membership_period', $payment->membership_period) === '1 Month' ? 'selected' : '' }}>1 Month</option>
                    <option value="3 Months" {{ old('membership_period', $payment->membership_period) === '3 Months' ? 'selected' : '' }}>3 Months</option>
                    <option value="6 Months" {{ old('membership_period', $payment->membership_period) === '6 Months' ? 'selected' : '' }}>6 Months</option>
                    <option value="12 Months" {{ old('membership_period', $payment->membership_period) === '12 Months' ? 'selected' : '' }}>12 Months</option>
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                @if($payment->invoice_file)
                    <div class="mb-2">
                        <a href="{{ asset('storage/' . $payment->invoice_file) }}" target="_blank" class="text-indigo-600 text-sm hover:underline">
