...
<div class="bg-white rounded-lg shadow-sm p-6 max-w-2xl">
    <h1 class="text-xl font-bold text-gray-800 mb-6">Edit Member: {{ $member->name }}</h1>
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" value="{{ $member->member_id }}" disabled
                    class="w-full px-3 py-2 border rounded-lg bg-gray-50 text-sm text-gray-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="whatsapp" value="{{ old('whatsapp', $member->whatsapp) }}" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
            </div>
            <div>
                ...
                @if($member->photo)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $member->photo) }}" alt="Current photo" class="w-16 h-16 rounded-full object-cover">
                    </div>
                @endif
            ...
            <div>
                ...
                <select name="membership_package" required
                    class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
                    <option value="Basic" {{ old('membership_package', $member->membership_package) === 'Basic' ? 'selected' : '' }}>Basic</option>
                    <option value="Standard" {{ old('membership_package', $member->membership_package) === 'Standard' ? 'selected' : '' }}>Standard</option>
                    <option value="Premium" {{ old('membership_package', $member->membership_package) === 'Premium' ? 'selected' : '' }}>Premium</option>
                    <option value="VIP" {{ old('membership_package', $member->membership_package) === 'VIP' ? 'selected' : '' }}>VIP</option>
                </select>
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Start Date *</label>
                    <input type="date" name="start_date" value="{{ old('start_date', $member->start_date->format('Y-m-d')) }}" required
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Expired Date *</label>
                    <input type="date" name="expired_date" value="{{ old('expired_date', $member->expired_date->format('Y-m-d')) }}" required
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm">
                </div>
            </div>
            <div>
